    ...
    switch (action.type) {
        ...
        case 'priorityUp':
            ...
            if(action.priority - 1 - 1 >= 0) {
                ...
                copyItems[action.priority - 1 - 1] = {...currentItem, priority: currentItem.priority - 1}
                copyItems[action.priority - 1] = {...swappedItem, priority: swappedItem.priority + 1}
    ...
    return (
        <div>
            ...
            <ul style={{ listStyleType: "none" }}>
                {items.map((item: Item) => {
                    return ( // TODO - Adding key property
                        <li><ItemDisplay
                            key={item.priority}
                            item={item}
                            deleteItem={() => dispatchItems({ type: 'deleteItem', priority: item.priority })}
                            priorityUp={() => dispatchItems({ type: 'priorityUp', priority: item.priority })}
                            priorityDown={() => dispatchItems({ type: 'priorityDown', priority: item.priority })}
                        /></li>)
                })}
            </ul>
        </div>
